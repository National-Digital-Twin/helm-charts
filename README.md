...
>```
> then unsintall `helm -n org-a uninstall ia-node-oidc`
> and try again.
...
Deploy OAuth2Proxy (with Redis) with Helm:  This is used to ensure all endpoints are uthenticated using keycloak, can be skipped for local/dev deployments, but you will need a redis
```sh
...
After Kafka is deployed, update the Kafka authentication secret and KafkaUser to work with SASL_PLAINTEXT for local development:

```sh
# Get the Kafka user password from the secret created by ia-node-kafka chart
KAFKA_PASSWORD=$(kubectl get secret kafka-ia-node-user -n org-a -o jsonpath='{.data.password}' | base64 -d)

# Update auth secret to use SASL_PLAINTEXT instead of SASL_SSL
kubectl create secret generic kafka-auth-config -n org-a --from-literal=kafka-config.properties="security.protocol=SASL_PLAINTEXT
sasl.mechanism=SCRAM-SHA-512
sasl.jaas.config=org.apache.kafka.common.security.scram.ScramLoginModule required \
    username=\"kafka-ia-node-user\" \
    password=\"${KAFKA_PASSWORD}\";" --dry-run=client -o yaml | kubectl apply -f -

# Add DescribeConfigs to source topics (knowledge, ontology) to prevent broker crashes
...
# Add consumer group permissions to KafkaUser for graph-server, federator-server, and management-node
kubectl patch kafkauser kafka-ia-node-user -n org-a --type='json' -p='[
  {
    "op": "add",
...
# Add ACLs for federated topics (used by federator-client and graph-server)
kubectl patch kafkauser kafka-ia-node-user -n org-a --type='json' -p='[
  {
    "op": "add",
    "path": "/spec/authorization/acls/-",
...
# Add cluster-level ACLs for metadata operations (required for AdminClient operations)
kubectl patch kafkauser kafka-ia-node-user -n org-a --type='json' -p='[
  {
    "op": "add",
    "path": "/spec/authorization/acls/-",
...
# Add DESCRIBE operations for federated topics (required by graph-server)
kubectl patch kafkauser kafka-ia-node-user -n org-a --type='json' -p='[
  {
    "op": "add",
    "path": "/spec/authorization/acls/-",
...
create namespace (if not done earlier)
...
>[!NOTE]
> a bug in the current published image has an issue publishing messages (GRPCClient.java 136-160)
...
```sh
# Copy Kafka authentication secret
kubectl get secret kafka-auth-config -n org-a -o yaml | sed 's/namespace: org-a/namespace: org-b/' | kubectl apply -f -
...
helm install test-msg ./charts/test-message-pod -n org-a \
  ...
  --set-string podAnnotations."traffic\\.sidecar\\.istio\\.io/excludeOutboundPorts"=9092
```
...
Install Kafka UI:
```sh
helm repo add kafka-ui https://provectuslabs.github.io/kafka-ui
...
#### jobRunr
inside the federtor client there is a jobrunr which controlls the robs for fetching new data from the server. you can expose the control dashboard for this and trigger jobs manually.
```sh
...
**Important:** The federator-client's gRPC connection is configured in the `connection-configuration.json` file within the Helm values. Ensure this points to the correct federator-server endpoint:
...
The federator-client writes messages back to org-b's Kafka (configured via `kafka.bootstrapServers` in client.properties):
```yaml
kafka.bootstrapServers=kafka-cluster-kafka-bootstrap.org-b.svc.cluster.local:9092
```

The graph-server in org-a also consumes from org-b's Kafka:
```yaml
kafkaCluster.bootstrapServers=kafka-cluster-kafka-bootstrap.org-b.svc.cluster.local:9092
```
